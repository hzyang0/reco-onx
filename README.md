...
当前运行时会连接 MySQL 8.4。用户画像、行为、实验分组、候选商品和库存都从数据库读取；这些是仓库内置的示例数据，方便在本地重复运行，不是生产用户数据。数据集包含 5 个特点鲜明的用户画像和 100 条商品、直播、广告候选，其中 80 条 goods 是 80 种独立商品，不使用同一商品的多个款式凑数。
